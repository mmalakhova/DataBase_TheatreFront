...
export default function Actors({
    ...
    const init = (currentPage) => {
        actorsService.getAll(currentPage - 1, recordPerPage)
            .then(response => {
    ...
    const handleChange = (e) => {
        ...
        actorsService.getAllSorted(currentPage - 1, recordPerPage, e.target.value)
            .then(response => {
                console.log('Actors data', response.data);
                setActors(response.data.content);
                setTotalPages(response.data.totalPages);
    ...
    const searchData = () => {
        actorsService.getAllFiltered(currentPage - 1, recordPerPage, searchName, searchCountry, searchGender)
            .then(response => {
                console.log('Actors data', response.data);
                setActors(response.data.content);
                setTotalPages(response.data.totalPages);
    ...
    return (
        <div className="content p-40">
            <div className="align-left justify-between">
                <h1 className="text-uppercase">актеры</h1>
                <Form style={{ marginLeft: 40, marginRight: 40 }}>

                    <Row>
                        {/* <InputGroup style={{ marginBottom: 10 }}>
                            <FormControl style={{marginRight:10, width:100}} onChange={searchChangeName} placeholder='Введите имя актера' name="search" value={searchName} />
            ...
            <div className="newsPanel d-flex flex-wrap">
                ...
                <Link to="/actors/add" style={{ marginLeft: 40, marginTop: 40, color: 'white' }} className="btn btn-dark mb-2">Добавить актера</Link>
